...
export default function StorageChart({ broker, refreshInterval = 30, autoRefresh = false, className }: StorageChartProps) {
  ...
  const transformStorageData = (data: StorageData, from: number, to: number, step: number): ChartDataPoint[] => {
    ...
    for (let timestamp = from; timestamp <= to; timestamp += step) {
      fullTimeline.push({
        ...
        store_messages_bytes: null,
        messages_dropped_per_sec: null,
        drops_last_minute_packets: null,
        drops_last_minute_bytes: null
      })
    }

    console.log(`Created full storage timeline: ${fullTimeline.length} points`)
  ...
  const fetchStorageData = useCallback(async () => {
    try {
      ...
      const transformedData = transformStorageData(data, from, to, step)
